...
//
// Clock note: Shell-facing logic runs on clk_main_a0 (250 MHz). Solver
// logic runs on clk_solver (15.625 MHz) generated by a CL-owned MMCME4_ADV
// from clk_main_a0. aws_clk_gen is instantiated with all clock groups
// disabled (CLK_GRP_A_EN=0) — it is only present for HDK elaboration
// compatibility. CDC crossings use xpm_cdc_single / AXI clock converters.
...
`include "cl_satswarm_defines.vh"
    ...
    // =========================================================================
    // Clocking IP (AWS_CLK_GEN) — all groups disabled.
    // Present for HDK elaboration compatibility only; no MMCMs instantiated.
    // =========================================================================
    ...
    // =========================================================================
    // Solver clock: CL-owned MMCM  clk_main_a0 (250 MHz) -> clk_solver (15.625 MHz)
    //
    // MMCM parameters: VCO = 250 * 5 = 1250 MHz (valid: 800-1600)
    //                  CLKOUT0 = 1250 / 80 = 15.625 MHz
    //
    // Vivado auto-propagates a generated clock through the MMCM and BUFG.
    // CDC crossings between clk_main_a0 and clk_solver are covered by
    // set_false_path in cl_timing_user.xdc.
    // =========================================================================
    logic clk_solver_unbuf, clk_solver_fb, mmcm_solver_locked;
    logic clk_solver;
    logic rst_solver_n;

    MMCME4_ADV #(
        .CLKFBOUT_MULT_F   (5.0),
        .CLKIN1_PERIOD      (4.0),      // 250 MHz
        .CLKOUT0_DIVIDE_F   (80.0),     // 1250 / 80 = 15.625 MHz
        .DIVCLK_DIVIDE      (1),
        .STARTUP_WAIT       ("FALSE")
    ) u_mmcm_solver (
        .CLKIN1     (clk_main_a0),
        .CLKIN2     (1'b0),
        .CLKINSEL   (1'b1),
        .CLKFBIN    (clk_solver_fb),
        .CLKFBOUT   (clk_solver_fb),
        .CLKOUT0    (clk_solver_unbuf),
        .LOCKED     (mmcm_solver_locked),
        .RST        (~rst_main_n_sync),
        .PWRDWN     (1'b0),
        .DADDR      (7'd0),
        .DCLK       (1'b0),
    ...
    BUFG u_bufg_solver (.I(clk_solver_unbuf), .O(clk_solver));

    // Solver reset: held until MMCM locks and shell reset releases.
    // 2-FF synchronizer into the clk_solver domain.
    logic rst_solver_n_q1, rst_solver_n_q2;
    always_ff @(posedge clk_solver or negedge rst_main_n_sync) begin
        if (!rst_main_n_sync) begin
            rst_solver_n_q1 <= 1'b0;
            rst_solver_n_q2 <= 1'b0;
        end else begin
            rst_solver_n_q1 <= mmcm_solver_locked;
            rst_solver_n_q2 <= rst_solver_n_q1;
        end
    end
    assign rst_solver_n = rst_solver_n_q2;

    // =========================================================================
    // AXI CDC for OCL (AXI-Lite)
    // Shell side runs on clk_main_a0; solver side runs on clk_solver.
    // =========================================================================
    ...
    cl_axi_clock_converter_light OCL_CDC (
        ...
        .m_axi_aclk    (clk_solver),
        .m_axi_aresetn (rst_solver_n),
    ...
    // =========================================================================
    // AXI CDC for DDR
    // Solver-side requests run on clk_solver; shell/sh_ddr side runs on clk_main_a0.
    // =========================================================================
    ...
    assign slv_ocl_awready = rst_solver_n;
    assign slv_ocl_wready  = rst_solver_n;
    ...
    assign slv_ocl_arready = rst_solver_n && !ocl_rd_pending;
    ...
    assign slv_pcis_awready = rst_solver_n && !pcis_aw_pending;
    ...
    assign slv_pcis_wready = rst_solver_n && pcis_aw_pending && pcis_wr_ready;
    ...
    // =========================================================================
    // =========================================================================
    // Virtual LED status: sync solver status (clk_solver) -> shell domain (clk_main_a0)
    // =========================================================================
    logic solver_done_15, solver_sat_15, solver_unsat_15;
    always_ff @(posedge clk_solver or negedge rst_solver_n) begin
        if (!rst_solver_n) begin
            solver_done_15  <= 1'b0;
            solver_sat_15   <= 1'b0;
            solver_unsat_15 <= 1'b0;
        end else begin
            solver_done_15  <= axil_rd_data[0];
            solver_sat_15   <= axil_rd_data[1];
            solver_unsat_15 <= axil_rd_data[2];
        end
    end

    logic solver_done_sync1, solver_done_sync2;
    logic solver_sat_sync1, solver_sat_sync2;
    logic solver_unsat_sync1, solver_unsat_sync2;
    always_ff @(posedge clk_main_a0 or negedge rst_main_n_sync) begin
        if (!rst_main_n_sync) begin
            solver_done_sync1  <= 1'b0;
            solver_done_sync2  <= 1'b0;
            solver_sat_sync1   <= 1'b0;
            solver_sat_sync2   <= 1'b0;
            solver_unsat_sync1 <= 1'b0;
            solver_unsat_sync2 <= 1'b0;
        end else begin
            solver_done_sync1  <= solver_done_15;
            solver_done_sync2  <= solver_done_sync1;
            solver_sat_sync1   <= solver_sat_15;
            solver_sat_sync2   <= solver_sat_sync1;
            solver_unsat_sync1 <= solver_unsat_15;
            solver_unsat_sync2 <= solver_unsat_sync1;
        end
    end

    assign cl_sh_status_vled = {13'b0, solver_unsat_sync2, solver_sat_sync2, solver_done_sync2};
